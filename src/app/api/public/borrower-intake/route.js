...
// validate and easy to mistype. Photos (borrower NIC/photo-proof and up to
// 2 guarantors' worth of the same) are accepted here too now, but stay
// optional — a slow connection or a borrower without their documents on
// hand shouldn't block getting the rest of the application in; an agent
// can still fill in whatever's missing when converting this to a real loan.
export async function POST(request) {
  try {
    ...
    let nicPhotoUrls = null;
    if (Array.isArray(nic_photos) && nic_photos.length > 0) {
      nicPhotoUrls = validateImageDataUrlArray(nic_photos);
      if (!nicPhotoUrls) {
    ...
    let photoProofUrls = null;
    if (Array.isArray(photo_proofs) && photo_proofs.length > 0) {
      photoProofUrls = validateImageDataUrlArray(photo_proofs);
      if (!photoProofUrls) {
    ...
    const [intake] = await db('borrower_intakes')
      .insert({
        ...
        date_of_birth: dob,
        nic_number: cleanNIC,
        nic_photo_urls: nicPhotoUrls ? JSON.stringify(nicPhotoUrls) : null,
        photo_proof_urls: photoProofUrls ? JSON.stringify(photoProofUrls) : null,
