...
/**
 * Start background interest check runner (runs every minute or hour)
 */
export function startInterestScheduler(intervalMs = 60 * 1000) {
  setInterval(async () => {
    try {
      await runInterestAccruals();
    } catch (err) {
      console.error('Error in scheduled interest accrual worker:', err);
    }
  }, intervalMs);
}
