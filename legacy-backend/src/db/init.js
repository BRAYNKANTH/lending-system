...
export async function initializeDatabase() {
  try {
    ...
    if (isFreshInstall) {
      ...
      // Installments (fixed repayment schedule/passbook — overlays the loan;
      // interest accrual timing is untouched and keeps running independently)
    ...
    await db.schema.createTable('ledger_entries', (table) => {
      ...
      table.string('account', 30).notNullable(); // 'cash_agent', 'loan_receivable', 'interest_revenue'
    ...
    // 7. Cash Remittances (Agent -> Office reconciliation)
    ...
    // 8. Guarantors (one per loan) — mirrors the STN "Guarantor Personal
    // Information" paper form
    ...
    const users = [
      ...
      },
      {
        ...
        name: 'Borrower Bandara',
        email: 'borrower@lend.com',
        phone: '+94778888888',
        password_hash: HASHED_PASSWORD,
        role: 'borrower'
      },
      {
        ...
        name: 'Borrower Chandana',
        email: 'borrower2@lend.com',
        phone: '+94779999999',
        password_hash: HASHED_PASSWORD,
        role: 'borrower'
      }
    ...
    // Daily accrual loan - Borrower Bandara
    ...
    // Weekly accrual loan - Borrower Chandana
    ...
    const loans = [
      {
        ...
        interest_rate: 2.00,
        interest_type: 'daily',
        current_balance: 87000.00, // 100k principal + 2k interest - 15k payment
        status: 'active',
        last_accrual_date: lastAccrualDaily,
        next_accrual_date: nextAccrualDaily,
      ...
      },
      {
        ...
        current_balance: 262500.00, // 250k principal + 12.5k interest
        status: 'active',
        last_accrual_date: lastAccrualWeekly,
        next_accrual_date: nextAccrualWeekly,
    ...
    // Seed Interest Accruals Log
    const accruals = [
      {
        id: 'i1111111-1111-1111-1111-111111111111',
        loan_id: 'b1111111-1111-1111-1111-111111111111',
        amount_accrued: 2000.00,
        calculation_log: 'Principal: 100000.00 | Rate: 2% | Type: daily',
        created_at: lastAccrualDaily
      },
      {
        id: 'i2222222-2222-2222-2222-222222222222',
        loan_id: 'b2222222-2222-2222-2222-222222222222',
        amount_accrued: 12500.00,
        calculation_log: 'Principal: 250000.00 | Rate: 5% | Type: weekly',
        created_at: lastAccrualWeekly
      }
    ];

    await db('interest_accruals').insert(accruals);
    console.log('Interest accruals seeded.');

    // Seed Ledger Entries
    const ledgers = [
      // Loan 1 Principal (Debit Receivable 100k)
      {
        loan_id: 'b1111111-1111-1111-1111-111111111111',
      ...
      // Loan 1 Interest (Debit Receivable 2k, Credit Revenue 2k)
      {
        loan_id: 'b1111111-1111-1111-1111-111111111111',
        account: 'loan_receivable',
        type: 'debit',
      ...
      // Loan 1 Repayment (Debit Cash Agent 15k, Credit Receivable 15k)
      {
        loan_id: 'b1111111-1111-1111-1111-111111111111',
        transaction_id: 'c1111111-1111-1111-1111-111111111111',
        account: 'cash_agent',
        type: 'debit',
        amount: 15000.00,
        created_at: new Date(baseDate.getTime() - 12 * 60 * 60 * 1000)
      ...
      // Loan 2 Principal (Debit Receivable 250k)
      {
        loan_id: 'b2222222-2222-2222-2222-222222222222',
        account: 'loan_receivable',
        type: 'debit',
      ...
      // Loan 2 Interest (Debit Receivable 12.5k, Credit Revenue 12.5k)
      {
        loan_id: 'b2222222-2222-2222-2222-222222222222',
        account: 'loan_receivable',
        type: 'debit',
    ...
    await db('ledger_entries').insert(ledgers);
    ...
    const audits = [
      {
        ...
        description: 'Lender Admin created a Daily loan of 100,000 LKR for Borrower Bandara.',
        created_at: new Date(baseDate.getTime() - 3 * 24 * 60 * 60 * 1000)
      },
      {
        actor_id: 'a1111111-1111-1111-1111-111111111111',
        action_type: 'CREATE_LOAN',
        description: 'Lender Admin created a Weekly loan of 250,000 LKR for Borrower Chandana.',
        created_at: new Date(baseDate.getTime() - 10 * 24 * 60 * 60 * 1000)
      },
      {
        actor_id: 'a2222222-2222-2222-2222-222222222222',
        action_type: 'RECORD_PAYMENT',
        description: 'Agent Amal collected 15,000 LKR for Borrower Bandara.',
        created_at: new Date(baseDate.getTime() - 12 * 60 * 60 * 1000)
      }
    ];

    await db('audit_logs').insert(audits);
    console.log('Audit logs seeded.');
    console.log('Database initialization complete!');
    } else {
      console.log('Database tables already exist. Checking for incremental schema migrations...');
    }

    // Additive migrations: bring an existing database up to date with new
    // columns/tables introduced after its initial creation. Safe to run on
    // every boot since every step is guarded by a hasTable/hasColumn check.
...
async function runIncrementalMigrations() {
  ...
  const hasRemittances = await db.schema.hasTable('remittances');
  if (!hasRemittances) {
    await db.schema.createTable('remittances', (table) => {
      table.uuid('id').primary().defaultTo(db.fn.uuid());
      table.uuid('agent_id').notNullable().references('id').inTable('users').onDelete('RESTRICT');
  ...
  const hasGuarantors = await db.schema.hasTable('guarantors');
  if (!hasGuarantors) {
    await db.schema.createTable('guarantors', (table) => {
      table.uuid('id').primary().defaultTo(db.fn.uuid());
      table.uuid('loan_id').notNullable().references('id').inTable('loans').onDelete('CASCADE');
